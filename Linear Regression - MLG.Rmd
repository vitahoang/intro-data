...
pulchritude and putative pedagogical productivity" [@Hamermesh2005] found that
...
[@Gelman2007].) The result is a data frame where each row contains a different
course and columns represent variables about the courses and professors.
...
1.  Is this an observational study or an experiment?
    ...
    <li>Observational study - **TRUE**</li>

    <li>Experiment</li>

    </ol>

2.  The original research question posed in the paper is whether beauty leads
    directly to the differences in course evaluations. Given the study design,
    should the question be rephrased? If so, how?

    <ol>

    <li>No, the question is worded accurately.</li>

    <li>Yes, revise wording to "Is there an association between beauty and
    course evaluations? - TRUE"</li>

    <li>Yes, revise wording to "Does beauty score increase the professor's
    course evaluations?"</li>

    <li>Yes, revise wording to "Does beauty score decrease the professor's
    course evaluations?"</li>
...
3.  Which of the following statements is **false** about the distribution of
    `score`?
    ...
    <li>25% of the students gave their professors a score of over 4.6. **-
    TRUE**</li>

    <li>11 of students gave a professor a score below 3.</li>

    <li>The left skewness of the data suggests that the students are less likely
    to rate the professors highly.</li>

    </ol>

```{r score-dist}
# type your code for Question 3 here, and Knit
hist(evals$score, breaks = 30)
median(evals$score)
nrow(evals[evals$score > 4.6,])/nrow(evals)
...
m1 = cor(evals[c(1,6,7,8,9,13,14,15,16,17,18,19)])
...
in the data frame with the approximate number of points on the scatterplot. Is
anything awry?

<div>

**Exercise:** Replot the scatterplot, but this time replace the `geom_point()`
layer with a `geom_jitter()` layer. (Use `?geom_jitter` to learn more.) What was
...
by average beauty rating and add the line to your plot using the following. If
...
The blue line is the model. The shaded gray area around the line tells us about
the variability we might expect in our predictions. To turn that off, use
`se = FALSE`.
...
```{r summary-model}
# type your code for the Exercise here, and Knit
m2 <- lm(score ~ bty_avg, data = evals)
summary(m2)
```

4.  Average beauty score is a statistically significant predictor of evaluation
    score.
...
5.  Use residual plots to evaluate whether the conditions of least squares
    regression are reasonable. Which of the following statements is an incorrect
    analysis of the residual plots and conditions?
    ...
    <li>Nearly normal residuals: Residuals are right skewed, but the sample size
    is large, so this may not be an important violation of conditions. **-
    TRUE**</li>

    <li>Independent observations: Classes sampled randomly, no order
    effect.</li>
...
#residuals plot: linear relationships between x(numerical) and y
plot(m2$residuals ~ evals$bty_avg)

#histogram: normally distributed with a mean of 0
hist(m2$residuals, breaks = 30)
qqnorm(m2$residuals)
qqline(m2$residuals)

#fitted plot: constant variability of residuals
plot(m2$residuals ~ m2$fitted)
plot(abs(m2$residuals) ~ m2$fitted)

#residuals plot: independent residuals
plot(m2$residuals)
```
...
6.  P-values and parameter estimates should only be trusted if the conditions
    for the regression are reasonable. Using diagnostic plots, we can conclude
    that the conditions for this model are reasonable.
...
**Exercise:** Print a summary of the multiple linear regression model. Is
`bty_avg` still a significant predictor of `score`? Has the addition of `gender`
...
7.  For two professors (one male and one female) who received the same beauty
    rating, the male professor is predicted to have the higher course evaluation
    score than the female.
...
of`genderfemale` has no deeper meaning. R simply codes the category that comes
first alphabetically as a $0$. (You can change the reference level of a
categorical variable, which is the level that is coded as a 0, using
the`relevel` function. Use `?relevel` to learn more.)
...
8.  Which of the following is the correct order of the three levels of rank if
    we were to order them from lowest predicted course evaluation score to
    highest predicted course evaluation score?
    ...
    <li>Tenure Track, Tenured, Teaching - **TRUE**</li>
...
Suppose we want to use the model we created earlier, `m_bty_gen` to predict the
evaluation score for a professor, Dr. Hypo Thetical, who is a male tenure track
...
**Exercise:** Check your suspicions from the previous exercise. Include the
model output in your response.

</div>

9.  Which of the following is the correct interpretation of the coefficient
    associated with the ethnicity variable. <br> Non-minority professors are
    expected on average to score ...

    <ol>

    <li>0.12 points lower than minority professors, all else held constant. -
    **TRUE**</li>

    <li>0.12 points higher than minority professors, all else held
    constant.</li>

    <li>0.02 points lower than minority professors, all else held constant.</li>

    <li>0.02 points higher than minority professors, all else held
    constant.</li>
...
model.) If not, what does this say about whether or not the dropped variable was
...
Now we try a different model selection method: adjusted $R^2$. Create a new
model, `m1`, where you remove `rank` from the list of explanatory variables.
Check out the adjusted $R^2$ of this new model and compare it to the adjusted
$R^2$ of the full model.

```{r eval=FALSE}
m_r1 <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_r1)$adj.r.squared
...
```{r eval=FALSE}
m_r2 = lm(score ~ rank + gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_r2)$adj.r.squared
summary(m_full)$adj.r.squared
```
...
# remove cls_profs
# remove cls_level
cat("full-2:", summary(lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + cls_students + cls_credits + bty_avg + pic_outfit + pic_color, data = evals))$adj.r.squared, "\n")

m_r3 = lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("rank:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + gender + language + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("ethnicity:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + language + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("gender:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + gender + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("language:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + gender + language + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("age:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + gender + language + age 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("cls_perc_eval:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("cls_students:", summary(m_r3)$adj.r.squared, "\n")


m_r3 = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + bty_avg 
             + pic_outfit + pic_color, data = evals)
cat("cls_credits:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_credits 
             + pic_outfit + pic_color, data = evals)
cat("bty_avg:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_color, data = evals)
cat("pic_outfit:", summary(m_r3)$adj.r.squared, "\n")

m_r3 = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit, data = evals)
cat("pic_color:", summary(m_r3)$adj.r.squared, "\n")



```

10. Elimination of which variable from the full model yielded the highest
    adjusted R-squared?
...
**Exercise:** The original paper describes how these data were gathered by
taking a sample of professors from the University of Texas at Austin and
including all courses that they have taught. Considering that each row
represents a course, could this new information have an impact on any of the
...
by Mine Çetinkaya-Rundel and Andrew Bray.
:::
